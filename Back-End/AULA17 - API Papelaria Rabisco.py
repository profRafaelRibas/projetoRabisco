...
def cadastro_produto():
    try:
        ...
        comandoSQL = 'INSERT INTO Produtos (nome, descricao, preco, quantidade) VALUES (%s,%s,%s,%s)'
...
def listar_produtos():
    try:
        ...
        if not produtos:
            return jsonify({'mensagem':'Não há produtos'}), 204
...
def get_produto(id_produto):
    try:
        ...
        comandoSQL = 'SELECT * FROM Produtos WHERE idProduto = %s'
        conexaoDB, cursorDB = conecta_db()
...
def update_produto():
    try:
        ...
        comandoSQL = 'UPDATE Produtos SET nome = %s, descricao = %s, preco = %s, quantidade = %s WHERE idproduto = %s'
        conexaoDB, cursorDB = conecta_db()
...
def delete_produto():
    try:
        ...
        comandoSQL = 'DELETE FROM Produtos WHERE idProduto = %s'
        conexaoDB, cursorDB = conecta_db()
        cursorDB.execute(comandoSQL, (id_produto,))
        conexaoDB.commit()
