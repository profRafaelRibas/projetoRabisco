...
def cadastrar_produto():
    ...
    comandoSQL = f'INSERT INTO Produtos VALUES (null,"{nome}","{descricao}",{preco},{quantidade})'
...
def listar_produtos():
    ...
    try:
        cursorDB = conexaoDB.cursor()
        cursorDB.execute('SELECT * FROM Produtos')
...
def get_produto(id_produto):
    cursorDB = conexaoDB.cursor()
    comandoSQL = f'SELECT * FROM Produtos WHERE idProduto = {id_produto}'
    cursorDB.execute(comandoSQL)
...
def altera_quantidade():
    ...
    try:
        comandoSQL = f'UPDATE Produtos SET quantidade = {nova_quantidade} WHERE idProduto = {id_produto}'
        cursorDB = conexaoDB.cursor()
        cursorDB.execute(comandoSQL)
        conexaoDB.commit()
    except mysql.connector.Error as erro:
...
def altera_preco():
    ...
    try:
        comandoSQL = f'UPDATE Produtos SET preco = {novo_preco} WHERE idProduto = {id_produto}'
        cursorDB = conexaoDB.cursor()
        cursorDB.execute(comandoSQL)
        conexaoDB.commit()
    except mysql.connector.Error as erro:
...
def excluir_produto():
    ...
    try:
        cursorDB = conexaoDB.cursor()
        comandoSQL = f'DELETE FROM Produtos WHERE idProduto = {id_produto}'
        cursorDB.execute(comandoSQL)
        conexaoDB.commit()
    except mysql.connector.Error as erro:
